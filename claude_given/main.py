...
MODEL_PATH = "fall_rf_v2.joblib"
TARGET_HZ = 200
WINDOW_SECONDS = 2.0
STEP_SECONDS = 0.5
STILLNESS_WINDOW_S = 2.0
WARNING_COUNTDOWN_S = 30.0
# v2: re-derived from a leave-one-subject-out precision/recall curve on the
# retrained model (see evaluation_report.md). 0.65 was carried over from the
# original (SisFall-trained) model and left this system missing ~90%+ of
# real falls on this hardware — 0.32 trades a higher raw trigger rate for
# much better sensitivity, and relies on ML_CONSECUTIVE_TRIGGERS_REQUIRED +
# the stillness/altitude fusion stage below to keep false alarms in check.
FALL_PROB_THRESHOLD = 0.32
ORIENTATION_CALIBRATION_S = 5.0
...
def calibrate_orientation(imu, duration_s: float = ORIENTATION_CALIBRATION_S):
    """Estimate the local 'resting' gravity direction as a unit vector, so
    extract_features() doesn't have to assume a fixed mounting orientation.
    Wearer should stand still, same as the barometer calibration below.
    Mirrors calibrate_barometer()'s pattern: average several raw samples."""
    print(f"Calibrating device orientation over {duration_s:.0f}s (stand still)...")
    samples = []
    t0 = time.time()
    while time.time() - t0 < duration_s:
        accel_g, _ = imu.read_raw()
        samples.append(accel_g)
        time.sleep(0.01)
    if not samples:
        print("[WARN] No orientation samples captured, defaulting to (0,0,-1).")
        return np.array([0.0, 0.0, -1.0])
    mean_vec = np.mean(np.array(samples), axis=0)
    gvec = mean_vec / (np.linalg.norm(mean_vec) + 1e-9)
...
def main():
    ...
    gravity_vec = calibrate_orientation(imu)
            ...
            if state == STATE_MONITORING:
                ...
                if len(buffer) == WINDOW_SAMPLES and samples_since_eval >= STEP_SAMPLES:
                    ...
                    if not validate_window(window_df):
                        ...
                        feats = extract_features(window_df, gravity_vec)
